package com.sf.web.si.customer.control;

import java.util.*;
import java.io.File;

import com.smartframework.web.core.util.*;
import com.smartframework.core.i18n.I18n;
import com.smartframework.web.core.control.BasicControl;
import com.smartframework.web.core.queryfilter.QueryFilter;
import com.smartframework.core.dao.page.PagingBean;
import com.sf.web.si.customer.service.ComFamilyInfoService;
import com.sf.web.si.customer.entity.ComFamilyInfo;
import com.smartframework.core.annotation.SmartComment;
import com.smartframework.core.smart.SmartView;
/**
 * @desc    ComFamilyInfoControl.java SmartWeb Code Plugin Created.
 * @date    2018/01/02
 */
@SmartComment("ComFamilyInfo实例增删改查接口层")
public class ComFamilyInfoControl extends BasicControl
{
	private ComFamilyInfoService 	comFamilyInfoService;//Service
	
	public ComFamilyInfoService getComFamilyInfoService()
	{
		return comFamilyInfoService;
	}
	
	public void setComFamilyInfoService(ComFamilyInfoService comFamilyInfoService)
	{
		this.comFamilyInfoService=comFamilyInfoService;
	}
	
	//CRUD 操作列表
	@SmartComment("保存ComFamilyInfo实例")
	public SmartView save(ComFamilyInfo comFamilyInfo)
	{
		//TODO 请先验证参数有效性再使用
		return this.dbAccess(this.comFamilyInfoService.save(comFamilyInfo));
	}
	
	@SmartComment("按ID列表删除ComFamilyInfo对象")
	public SmartView delete(List<Integer> ids)
	{
		//TODO 请先验证参数有效性再使用
		return this.dbAccess(this.comFamilyInfoService.delete(ids,false));
	}
	
	@SmartComment("按ID列表彻底删除ComFamilyInfo对象")
	public SmartView deletePermanently(List<Integer> ids)
	{
		//TODO 请先验证参数有效性再使用
		return this.dbAccess(this.comFamilyInfoService.delete(ids,false));
	}
	
	@SmartComment("按ID获取单个ComFamilyInfo实例")
	public SmartView loadOne(Integer id)
	{
		//TODO 请先验证参数有效性再使用
		return SmartView.SUCCESS(this.comFamilyInfoService.get(id));
	}
	
	//method标识是否执行导出,并根据pageSize<0确定是否全部导出
	@SmartComment("获取ComFamilyInfo实例的分页查询结果")
	public SmartView list(String name,String phone,Integer customerId,Integer agegt,Integer agelt,Integer pageSize,Integer pageIndex,String columns,String method)
	{
		QueryFilter queryFilter = new QueryFilter();
		//在这里根据输入的查询参数构建QueryFilter实例并执行 Service中默认SQL对象的别名是 e
		if(!ParameterUtils.isEmptyOrNull(name))
		{
			queryFilter.addStringFieldLikeCondition("e.name", name);
		}
		if(!ParameterUtils.isEmptyOrNull(phone))
		{
			queryFilter.addStringFieldLikeCondition("e.phone", phone);
		}
		if(!ParameterUtils.isEmptyOrNull(customerId))
		{
			queryFilter.addNumberFieldEqualCondition("e.customer_id", customerId);
		}
		if(!ParameterUtils.isEmptyOrNull(agegt))
		{
//			int agegt_int = 0;
//			try {
//			    agegt_int = Integer.valueOf(agegt).intValue();
//			} catch (NumberFormatException e) {
//			    e.printStackTrace();
//			}
			queryFilter.addDateFieldLessEqualCondition("e.birthday", DateUtil.addYears(new Date(), -1 * agegt));
		}
		if(!ParameterUtils.isEmptyOrNull(agelt))
		{
//			int agelt_int = 0;
//			try {
//			    agelt_int = Integer.valueOf(agelt).intValue();
//			} catch (NumberFormatException e) {
//			    e.printStackTrace();
//			}
			queryFilter.addDateFieldGreaterEqualCondition("e.birthday", DateUtil.addYears(new Date(), -1 * agelt));
		}
		if(pageSize==null||pageIndex==null)
		{
			return SmartView.ERROR(I18n.message("error.parameter.empty","分页参数{0}不能为空",new Object[]{"pageIndex,pageSize"}));
		}
		PagingBean page = this.comFamilyInfoService.queryRecords(queryFilter,pageSize,pageIndex);
		// TODO 导出时使用 List<HeaderConfig> headers = JsonUtil.getBeanListByJsonArray(columns, HeaderConfig.class); 
		// ExcelDataUtil.createExcelByMapList(page.getData(),headers).write(outputStream);
		//在Service中构建PagingBean实例
		return this.pageView(page);
		
	}
	
	//method标识是否执行导出,并根据pageSize<0确定是否全部导出
	@SmartComment("获取ComFamilyInfo按照客户过滤的查询结果")
	public SmartView listMembers()
	{
		QueryFilter queryFilter = new QueryFilter();
		PagingBean page = this.comFamilyInfoService.queryRecords(queryFilter,100,1);
		// TODO 导出时使用 List<HeaderConfig> headers = JsonUtil.getBeanListByJsonArray(columns, HeaderConfig.class); 
		// ExcelDataUtil.createExcelByMapList(page.getData(),headers).write(outputStream);
		//在Service中构建PagingBean实例
		return this.pageView(page);
	}
		
	//参数名 file 请勿轻易修改
	@SmartComment("Excel文件导入")
	public SmartView importFile(File file)
	{
		return SmartView.SUCCESS();
	}
}
